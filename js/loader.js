...
function injectLoader() {
    ...
    // Fallback if logo fails (try relative first)
    const img = document.getElementById('loader-logo-img');
    if (img) {
        img.onerror = () => {
            img.src = './assets/logo.png'; // Try relative path
            img.onerror = () => {
                // If relative also fails, try to extract from current path
                const currentPath = window.location.pathname;
                const basePath = currentPath.substring(0, currentPath.lastIndexOf('/') + 1);
                img.src = basePath + 'assets/logo.png';
                img.onerror = () => { img.style.opacity = '0'; }; // Hide if all fail
            };
        };
    }
}
...
window.addEventListener('load', () => {
    // Increased timeout to ensure content is visible before hiding loader
